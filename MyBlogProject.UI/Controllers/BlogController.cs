...
namespace MyBlogProject.UI.Controllers
{
    public class BlogController : Controller
    {
        ...
        {
            var blogs = blogManager.ListQueryable().Include("Category").Where(b => b.Owner.Id == CurrentSession.User.Id).OrderByDescending(b => b.ModifiedOn);
        ...
        {
            var blogs = likeManager.ListQueryable().Include("LikedUser").Include("Blog").Where(x => x.LikedUser.Id == CurrentSession.User.Id).Select(x => x.Blog).Include("Category").Include("Owner").OrderByDescending(x => x.ModifiedOn);
